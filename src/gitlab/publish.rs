...
impl GitLabClient {
    ...
    async fn publish_review(
        ...
        // Check them before any writes, even when an inline target is available.
        let mut checked_commits = HashSet::new();
        let mut commit_fingerprints = HashSet::new();
        for item in &remaining {
            if let Some(commit) = item_commit(item, &commits)
                && checked_commits.insert(commit.id.as_ref().to_string())
            {
                match self
