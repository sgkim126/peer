...
impl CacheKey {
    pub fn from_params<T>(namespace: impl Into<String>, params: &T) -> Result<Self, CacheKeyError>
    where
        T: Serialize,
    {
...
mod tests {
    ...
    fn hashes_serializable_parameters_deterministically() {
        let first = CacheKey::from_params("check", &serde_json::json!({"a": 1})).unwrap();
        let second = CacheKey::from_params("check", &serde_json::json!({"a": 1})).unwrap();
    ...
    #[test]
    fn namespace_does_not_change_params_hash() {
        let first = CacheKey::from_params("check", &"params").unwrap();
        let second = CacheKey::from_params("other-check", &"params").unwrap();
